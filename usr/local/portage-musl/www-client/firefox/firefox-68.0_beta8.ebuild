...
src_configure() {
	...
	#mozconfig_annotate '' --target="x86_64-gentoo-linux-musl"
	#mozconfig_annotate '' --host="${CBUILD:-x86_64-gentoo-linux-musl}"
	...
	if tc-is-cross-compiler && use elibc_musl ; then
		mozconfig_annotate 'cross-compiling, specify Clang/LLVM paths' --with-libclang-path=/usr/lib/llvm/8/lib64
		mozconfig_annotate 'cross-compiling, specify Clang/LLVM paths' --with-clang-path=/usr/lib/llvm/8/bin/clang-8
	fi
...
src_install() {
	...
	# toolkit/components/remote/nsRemoteService.cpp:L160 if no dbus, and no X11, segfault/assertion error
	# TODO: report bug / fix
	#if use wayland && ! use dbus ; then
	#	cat ${FILESDIR}/disable-e10s.js \
