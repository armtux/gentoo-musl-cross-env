...
src_configure() {
	...
	rust_targets="${rust_targets},\"x86_64-unknown-linux-musl\""
	#rust_targets="\"x86_64-gentoo-linux-musl\""
	...
	cat <<- EOF > "${S}"/config.toml
		...
		channel = "stable"
		rpath = false
		musl-root = "/usr/x86_64-unknown-linux-musl/usr"
	...
	cat <<- EOF >> "${S}"/config.env
		CFLAGS_x86_64-unknown-linux-musl=-O2 -march=native
	EOF
